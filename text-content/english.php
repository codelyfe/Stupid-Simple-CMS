<?php
// ADMIN
$admintitle = "<i class='fa-solid fa-blog'></i> (ADMIN) Stupid Simple CMS ( Blogger )";
$loginpagegreeting = "Join the virtual story today!";
$submitanarticle = "Submit an Article";
$logoutbutton = "Logout <i class='fa-solid fa-right-from-bracket'></i>";
$blogbutton = "Blog";
// USER
$sitetitle = "<i class='fa-solid fa-blog'></i> Stupid Simple CMS ( Blogger )";
$category = "Category: ";
$createdat = "Created at: <i class='fa-regular fa-clock'></i> ";
$searchbutton = "<i class='fa-solid fa-magnifying-glass'></i> Search";
$readmore="READ MORE <i class='fa-solid fa-up-right-from-square'></i>";
$goback="<i class='fa-regular fa-hand-point-left'></i> GO BACK";